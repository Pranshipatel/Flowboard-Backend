...
try {
    ...
    $totalLines = $coveredLines + [double]$lineCounter.missed
    if ($totalLines -le 0) {
        throw "JaCoCo XML report has no measurable lines: $coverageReport"
    }

    $lineCoverage = [math]::Round(($coveredLines / $totalLines) * 100, 2)
    Write-Host "Notification service JaCoCo line coverage: $lineCoverage% ($coveredLines/$totalLines lines covered)"

    if ($lineCoverage -lt $MinimumLineCoverage) {
        throw 'Notification service line coverage is ' + $lineCoverage + '%, below required ' + $MinimumLineCoverage + '%. SonarQube scan was not published.'
    }

    if ($lineCoverage -gt $MaximumLineCoverage) {
        throw 'Notification service line coverage is ' + $lineCoverage + '%, above allowed ' + $MaximumLineCoverage + '%. SonarQube scan was not published.'
    }
