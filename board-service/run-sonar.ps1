...
try {
    ...
    Write-Host "Board service JaCoCo line coverage: $lineCoverage% ($coveredLines/$totalLines lines covered)"

    if ($lineCoverage -lt $MinimumLineCoverage) {
        throw 'Board service line coverage is ' + $lineCoverage + '%, below required ' + $MinimumLineCoverage + '%. SonarQube scan was not published.'
    }

    if ($lineCoverage -gt $MaximumLineCoverage) {
        throw 'Board service line coverage is ' + $lineCoverage + '%, above allowed ' + $MaximumLineCoverage + '%. SonarQube scan was not published.'
    }
